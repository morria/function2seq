...
class Context:
    ...
    def subtoken_counter(self) -> Counter[str]:
        """
        Get a dictionary mapping each token (from the start and end terminals)
        to it's occurance count.
        """
        return self.terminal_start_subtokens.subtoken_counter(
        ) + self.terminal_end_subtokens.subtoken_counter()
...
class TargetContexts:
    ...
    def sample_and_pad_contexts(
        self, context_count: int, padded_context_count: Optional[int] = None
    ) -> None:
        """
        ...
        if padded_context_count is not None and len(
                self.contexts) < padded_context_count:
            self.contexts = self.contexts + [
                Context(Subtokens([]), [], Subtokens([]))
                for _ in range(padded_context_count - len(self.contexts))
            ]
